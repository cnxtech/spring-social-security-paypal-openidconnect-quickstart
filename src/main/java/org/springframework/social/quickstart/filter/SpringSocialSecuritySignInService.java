 ...
 * Invoked at the end of a provider sign-in attempt to sign-in the local user account associated
 * with the provider user account.
 * 
...
public class SpringSocialSecuritySignInService implements SignInAdapter {
	...
	public String signIn(String localUserId, Connection<?> connection, NativeWebRequest nativeWebRequest) {
		...
		nativeWebRequest.setAttribute(SIGN_IN_DETAILS_SESSION_ATTRIBUTE_NAME,
				new SpringSocialSecuritySignInDetails(localUserId, connection.createData()), RequestAttributes.SCOPE_SESSION);
		return null;
	}
}
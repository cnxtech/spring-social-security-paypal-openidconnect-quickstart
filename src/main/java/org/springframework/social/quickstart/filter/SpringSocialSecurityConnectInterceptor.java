...
public class SpringSocialSecurityConnectInterceptor<S> extends EnsureUniqueConnectInterceptor<S> implements ConnectInterceptor<S> {
	...
	public void postConnect(Connection<S> connection, WebRequest webRequest) {
		...
		if (!existingAuthorities.contains(newAuthority)) {
			...
			Authentication newAuthentication = authenticationFactory.updateAuthenticationForNewConnection(authentication, connection);
	...
	@Override
	public void preConnect(ConnectionFactory<S> arg0, MultiValueMap<String, String> arg1, WebRequest arg2) {
		// No-op
	}
